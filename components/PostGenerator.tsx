  ...
  const handleGenerate = async (bp: ContentBlueprint) => {
    ...
    try {
      const promises = bp.personaIds.map(async (personaId) => {
        ...
        if (!persona) return { id: personaId, text: "Error: Persona not found" };
        const text = await generateBlueprintContent(bp, persona, 'gemini-2.5-flash', effectiveConfig, language);
